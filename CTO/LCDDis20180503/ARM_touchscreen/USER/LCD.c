...
void showzifularge(unsigned int x,unsigned int y,char val,unsigned int dcolor,unsigned int bgcolor)
{  
	u16 i,j;
	u8 temp=0;
	u16 x0=x;
	...
	for(j=0;j<step;j++)
	{
		...
		for(i=0;i<8;i++)
		{
				...
				if((x1-x0)==(lengthValue))
				{
					...
					y1++;
					if(y1>=LCD_H)
					{POINT_COLOR=bgcolor;return;}//超区域了
					else if(y1-y0>=50)
					return;
					else
					break;
	...
	while(*str!='\0')
	{
	  if(size ==1 )
		{showzifu(x1,y1,*str,dcolor,bgcolor);
	  	...
	  	{showzifularge(x1,y1,*str,dcolor,bgcolor);
			x1+=38;	 }
...
void inttostr(int dd,char *str)
{
	if((dd/10000)==0)
		...
		str[0]=dd/10000+48;
	if(((dd/10000)==0)&&((dd/1000)==0))
		str[1]=32; 
	else 
		str[1]=(dd/1000)-((dd/10000)*10)+48;
	if(((dd/10000)==0)&&((dd/1000)==0)&&((dd/100)==0))
		str[2]=32; 
	else 
