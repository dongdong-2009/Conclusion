...
u16 PvRH=600,PvTe=255,SvTe=0,PvCO2=0,SvCO2=0,PSvTe=50, PSVCO2value=60;
u16 TempData=10;
	...
	while(1)
	{
		...
		if (getdataflag!=0)
		{
			 ...
			 for(cnti=0;cnti<20;cnti++)
			{
			 ...
			 if  (PCO2valueO>=266.0)
		    //	PCO2value   = rint(0.262*(PCO2valueO)+720)   ;
			//	PCO2value   = rint(0.59*(PCO2valueO)+519)   ;
			 	PCO2value   = rint(0.90*(PCO2valueO)+340)   ;
			 else
		       // PCO2value =rint(2.60*(PCO2valueO)+90.3) ;
			       PCO2value =rint(2.10*(PCO2valueO)+20.4) ;
			 // PCO2value	 =PCO2valueO;
			 if((IC1sum[0]==0)|(IC2valueav==40))
			 PCO2value =400;
			//155 503 266/798 		634/895	 580/	 629/
			 IC1sumvaluesum =0;
			 IC2sumvaluesum =0;
		}
    	...
    	if(	PCin(10)==0)
		{
			...
			else if(Oflag==1)
			{
				...
				switch(touchcase)
				{
					...
					{
					  Tim4SP();
						pageFinish =PAGE1(PvRH,PvTe,TempData);
				    ...
				    if  (PAGEflag==2)
					{	Tim4SP();
						pageFinish = PAGE1(PvRH,PvTe,TempData);PAGEflag=1;
					...
					case 10:if ((PAGEflag==2)&&(PSvTe>0))
					{
						PSvTe--;
						if(PSvTe==99)
						LCD_Fill(50,100,180,180,BLUEBG);
						inttostr(PSvTe,ss);
            showzifustr(0,120,ss,0,BLUEBG,2);
					}
					break;
