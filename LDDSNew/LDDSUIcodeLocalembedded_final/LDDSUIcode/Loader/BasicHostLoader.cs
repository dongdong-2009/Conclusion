namespace Loader
{
	...
	public class BasicHostLoader: BasicDesignerLoader
	{
        ...
        public List<IComponent> mComponents = new List<IComponent>();
		#region Constructors
        ...
        private object LoadObject(XmlNode node, ArrayList errors)
        {
            ...
            if (typeof(IComponent).IsAssignableFrom(type))
            {
                ...
                {
                    instance = host.CreateComponent(type);
                    mComponents.Add((IComponent)instance);
                }
                else
                {
                    instance = host.CreateComponent(type, nameAttr.Value);
            ...
            foreach (XmlNode childNode in node.ChildNodes)
            {
                if (childNode.Name.Equals("Object"))
                {
                    ...
                    if (childList != null)
                    {
                        object childInstance = LoadObject(childNode, errors);
        ...
        public void Save(string spath)
        {
            ...
            {
                MessageBox.Show("存储过程中出现错误: " + ex.ToString());
            }
        }
		public void Save()
		{
