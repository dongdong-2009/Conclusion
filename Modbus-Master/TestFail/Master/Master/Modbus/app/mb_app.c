...
{
	MB_EventType_t sta=MB_EventState();   //读取当前状态
...
void eMBRegInputCB(mb_pro_t *p_prot)
{
  ...
  iRegIndex = ( int )( p_prot->startadd - REG_INPUT_START );
	while( p_prot->regN > 0 )
	{
		*p_prot->databuff++ = ( unsigned char )( usRegInputBuf[iRegIndex] >> 8 );
		*p_prot->databuff++ = ( unsigned char )( usRegInputBuf[iRegIndex] & 0xFF );
		iRegIndex++;
		p_prot->regN--;
	}
}
...
{
	int iRegIndex;
	iRegIndex = ( int )( p_prot->startadd - REG_HOLDING_START );   //地址偏移量

	while(p_prot->regN > 0)
	{
		if(p_prot->mode==1)                //Read 模式
		{
			*p_prot->databuff++ = ( unsigned char )( usRegHoldingBuf[iRegIndex] >> 8 );    //数据高位
			*p_prot->databuff++ = ( unsigned char )( usRegHoldingBuf[iRegIndex] & 0xFF );  //数据低位
		}
		else if(p_prot->mode==2)		//高八位在前，低八位在后   写模式
		{
//			usRegHoldingBuf[iRegIndex]=*p_prot->databuff++ <<8;    
