...
uint8_t _u8TransmitBufferIndex;                              ///< Transmit buffer index>
uint16_t u16TransmitBufferLength;                            ///< Transmit buffer length>
uint8_t _u8ResponseBufferIndex;                              ///< Reponse buffer index>
uint8_t _u8ResponseBufferLength;                             ///< Reponse buffer index>
...
uint8_t ModbusMaster_ModbusMasterTransaction(uint8_t u8MBFunction)
{
  ...
  switch(u8MBFunction)
  {
	...
	switch(u8MBFunction)
  {
		//这个应该是写Output Value, 0XFF00---1,0X0000---0
    case ku8MBWriteSingleCoil:
      u8ModbusADU[u8ModbusADUSize++] = highByte(_u16WriteQty);    //寄存器数量高位 
      u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteQty);     //寄存器数量低位
      break;
      
    case ku8MBWriteSingleRegister:
      u8ModbusADU[u8ModbusADUSize++] = highByte(_u16TransmitBuffer[0]);  //数据高位
      u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16TransmitBuffer[0]);   //数据低位
      break;
      
    case ku8MBWriteMultipleCoils:
      u8ModbusADU[u8ModbusADUSize++] = highByte(_u16WriteQty);       //寄存器数量高位
      u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteQty);       //寄存器数量低位
      u8Qty = (_u16WriteQty % 8) ? ((_u16WriteQty >> 3) + 1) : (_u16WriteQty >> 3);  //判断所需要的字节数
    ...
    case ku8MBWriteMultipleRegisters:
      ...
      u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteQty);
			u8ModbusADU[u8ModbusADUSize++] = lowByte(_u16WriteQty << 1);   //写的字节数，=寄存器数量*2
  ...
  for (i = 0; i < u8ModbusADUSize; i++)
  {
	...
	u8ModbusADU[u8ModbusADUSize++] = highByte(u16CRC);    //CRC校验位高位
  u8ModbusADU[u8ModbusADUSize++] = lowByte(u16CRC);     //CRC校验位低位
  u8ModbusADU[u8ModbusADUSize] = 0;                     //清零
	
	// flush receive buffer before transmitting request
	//发送之前，清空缓存区
  Modbus_Master_Rece_Flush();
	
	//串口发送数据，para1---发送的数据，para2---发送数据长度
	//Modbus_Master_Write(u8ModbusADU,u8ModbusADUSize);
	UsartSendData(u8ModbusADUSize,(char*)u8ModbusADU);
	
	u8ModbusADUSize = 0;
	// loop until we run out of time or bytes, or an error occurs
  u32StartTime = Modbus_Master_Millis();
	while (u8BytesLeft && !u8MBStatus)   //异常处理
  {
    ...
    if (u8ModbusADUSize == 5)
    {
      ...
      if (u8ModbusADU[0] != _u8MBSlave)    //Slave ID 不正确
      {
      ...
      if ((u8ModbusADU[1] & 0x7F) != u8MBFunction)    //功能码是否正确
      {
      ...
      switch(u8ModbusADU[1])
      {
        ...
        case ku8MBReadInputRegisters:break;
        case ku8MBReadHoldingRegisters:break;
        case ku8MBReadWriteMultipleRegisters:
          u8BytesLeft = u8ModbusADU[2];
          break;
          
        case ku8MBWriteSingleCoil:break;
        case ku8MBWriteMultipleCoils:break;
        case ku8MBWriteSingleRegister:break;
        case ku8MBWriteMultipleRegisters:
          u8BytesLeft = 3;
          break;
...
//    }
  ...
  if (!u8MBStatus && u8ModbusADUSize >= 5)
  {
    ...
    // verify CRC  校验码高位在前，低位在后
    if (!u8MBStatus && (lowByte(u16CRC) != u8ModbusADU[u8ModbusADUSize - 1] ||
      highByte(u16CRC) != u8ModbusADU[u8ModbusADUSize - 2]))
    {
...
/*
uint8_t ModbusMaster_readWriteMultipleRegisters(uint16_t u16ReadAddress, uint16_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  _u16WriteQty = _u8TransmitBufferIndex;
