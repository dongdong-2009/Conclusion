...
void  OSStart (void)
{
    if (OSRunning == OS_FALSE) {                     /* 若运行状态未开始,则执行下面代码 开始运行多任务 */
        OS_SchedNew();                               /* 查找最高优先级任务进入就绪                     */
        OSPrioCur     = OSPrioHighRdy;               /* 当前优先级指向就绪任务的最高优先级             */
        OSTCBHighRdy  = OSTCBPrioTbl[OSPrioHighRdy]; /* 指向最高优先级                                 */
        OSTCBCur      = OSTCBHighRdy;                /* 当前TCB指向最高(优先级)就绪的TCB               */
        OSStartHighRdy();                            /* 执行目标代码，开始任务(OSRunning = OS_TRUE)    */
    }
}
/*$PAGE*/
/*
...
*                                          （初始化事件控制块）
* Description: 这个函数会被OSInit()调用来初始化空闲ECB(事件控制块)。
*
* Arguments  : none
*
* Returns    : none
...
static  void  OS_InitEventList (void)
{
    ...
    for (ix = 0u; ix < (OS_MAX_EVENTS - 1u); ix++) {        /* 初始化空ECB(事件控制块)                 */
        ix_next = ix + 1u;
        pevent1 = &OSEventTbl[ix];
        pevent2 = &OSEventTbl[ix_next];
        pevent1->OSEventType    = OS_EVENT_TYPE_UNUSED;
        pevent1->OSEventPtr     = pevent2;                  /* 指向下一消息或队列                      */
#if OS_EVENT_NAME_EN > 0u
        pevent1->OSEventName    = (INT8U *)(void *)"?";     /* 未知ECB名称                             */
#endif
    }
    pevent1                         = &OSEventTbl[ix];
    pevent1->OSEventType            = OS_EVENT_TYPE_UNUSED;
    pevent1->OSEventPtr             = (OS_EVENT *)0;        /* 最后一事件指针指向空                    */
#if OS_EVENT_NAME_EN > 0u
    pevent1->OSEventName            = (INT8U *)(void *)"?"; /* 未知ECB名称                             */
#endif
    OSEventFreeList                 = &OSEventTbl[0];       /* 空ECB指针指向OSEventTbl[0]              */
#else
    OSEventFreeList                 = &OSEventTbl[0];       /* 如果只有一个事件控制块···               */
    OSEventFreeList->OSEventType    = OS_EVENT_TYPE_UNUSED;
    OSEventFreeList->OSEventPtr     = (OS_EVENT *)0;
#if OS_EVENT_NAME_EN > 0u
    OSEventFreeList->OSEventName    = (INT8U *)"?";         /* 未知ECB名称                             */
#endif
#endif
#endif
}
...
*                                           （初始化各项变量）
* Description: 这个函数会被OSInit()调用来初始化各项变量。
*
* Arguments  : none
*
* Returns    : none
...
{
#if OS_TIME_GET_SET_EN > 0u
    OSTime                    = 0uL;                       /* 清零32位系统时钟                         */
#endif

    OSIntNesting              = 0u;                        /* 清零中断嵌套层数                         */
    OSLockNesting             = 0u;                        /* 清零调度锁                               */

    OSTaskCtr                 = 0u;                        /* 清零任务计数器                           */

    OSRunning                 = OS_FALSE;                  /* 表明多任务未开始(OS运行标志无效)         */

    OSCtxSwCtr                = 0u;                        /* 清零上下文切换计数器                     */
    OSIdleCtr                 = 0uL;                       /* 清零32位空闲任务计数器                   */
...
*                                           （初始化就绪列表）
* Description: 这个函数被OSInit()调用来初始化就绪表。
*
* Arguments  : none
*
* Returns    : none
...
static  void  OS_InitRdyList (void)
{
    ...
    OSTCBHighRdy  = (OS_TCB *)0;                           /* 初始化最高优先级TCB(任务控制块)          */
...
static  void  OS_InitTaskIdle (void)
{
    ...
    (void)OSTaskCreateExt(OS_TaskIdle,
                          (void *)0,                                 /* 没有参数指向OS_TaskIdle()            */
                          &OSTaskIdleStk[OS_TASK_IDLE_STK_SIZE - 1u],/* 设置堆栈(栈顶)                       */
                          OS_TASK_IDLE_PRIO,                         /* 设置为最低优先级                     */
                          OS_TASK_IDLE_ID,
                          &OSTaskIdleStk[0],                         /* 设置堆栈(栈底)                       */
                          OS_TASK_IDLE_STK_SIZE,
                          (void *)0,                                 /* 无TCB扩展参数                        */
                          OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR);/* 使能选项stack checking + clear stack */
    #else
    (void)OSTaskCreateExt(OS_TaskIdle,
...
*                                          （初始化任务控制块）
* Description: 这个函数会被OSInit()调用来初始化空闲TCB(任务控制块)。
*
* Arguments  : none
*
* Returns    : none
...
static  void  OS_InitTCBList (void)
{
    ...
    OS_MemClr((INT8U *)&OSTCBTbl[0],     sizeof(OSTCBTbl));      /* 清除所有TCB(任务控制块)            */
    OS_MemClr((INT8U *)&OSTCBPrioTbl[0], sizeof(OSTCBPrioTbl));  /* 清除优先级表                       */
    for (ix = 0u; ix < (OS_MAX_TASKS + OS_N_SYS_TASKS - 1u); ix++) {    /* 初始化空TCBs列表            */
        ix_next =  ix + 1u;
        ptcb1   = &OSTCBTbl[ix];
        ptcb2   = &OSTCBTbl[ix_next];
        ptcb1->OSTCBNext = ptcb2;
#if OS_TASK_NAME_EN > 0u
        ptcb1->OSTCBTaskName = (INT8U *)(void *)"?";             /* 未知TCB名称                        */
#endif
    }
    ptcb1                   = &OSTCBTbl[ix];
    ptcb1->OSTCBNext        = (OS_TCB *)0;                       /* 最后OS_TCB指向的"空"               */
#if OS_TASK_NAME_EN > 0u
    ptcb1->OSTCBTaskName    = (INT8U *)(void *)"?";              /* 未知TCB名称                        */
#endif
    OSTCBList               = (OS_TCB *)0;                       /* 初始化TCB链表                      */
...
*                                              IDLE TASK
*                                     （空任务，OS初始化时被创建）
* Description: 这是系统内部任务，在没有任务占用CPU资源时运行该任务
*
*
* Arguments  : none
*
...
{
#if OS_CRITICAL_METHOD == 3u                     /* 若临界模式 = 3u,则给CPU_SR分配空间                 */
    OS_CPU_SR  cpu_sr = 0u;
#endif
    ...
    for (;;) {
        ...
        OSIdleCtr++;                             /* 空闲统计计数+1                                     */
        OS_EXIT_CRITICAL();
        OSTaskIdleHook();                        /* 调用用户定义的空闲HOOK函数                         */
    }
}
/*$PAGE*/
/*
...
*                                           INITIALIZE TCB
*                                        （初始化任务控制块）
* Description: 这个uC/OS-II内部函数， 用于创建任务时初始化TCB.
*              (查看OSTaskCreate() and OSTaskCreateExt()).
*
* Arguments  : prio          任务的优先级
*
*              ptos          指向栈顶的指针,保存在OS_TCB的OSTCBStkPrt中.
*                            Note that the top-of-stack corresponds to a
*                            'high' memory location is OS_STK_GROWTH is set to 1 and a 'low' memory
*                            location if OS_STK_GROWTH is set to 0.  Note that stack growth is CPU
*                            specific.
*
*              pbos          指向栈底的指针,保存在OS_TCB的OSTCBStkBottom中.
*                            A NULL pointer is passed if called by'OSTaskCreate()'.
*
*              id            任务的 ID (0..65535)
*
*              stk_size      堆栈的大小 (在“堆栈单元”)，保存在OS_TCB的OSTCBStkSize中.
*                            If the stack units are INT8Us, then, 'stk_size' contains the number of bytes for the stack.
*                            If the stack units are INT32Us then, the stack contains '4 * stk_size' bytes.  The stack
*                            units are established by the #define constant OS_STK which is CPU
*                            specific.  'stk_size' is 0 if called by 'OSTaskCreate()'.
*
*              pext          指向内存区域扩展TCB的指针，保存在OS_TCB的OSTCBExtPtr中.
*                            This allows you to store the contents of floating-point
*                            registers, MMU registers or anything else you could find useful during a
*                            context switch.  You can even assign a name to each task and store this name
*                            in this TCB extension.  A NULL pointer is passed if called by OSTaskCreate().
*
*              opt           OS_TCB的选择项，保存在OS_TCB的OSTCBOpt中.
*                            options as passed to 'OSTaskCreateExt()' or,0 if called from 'OSTaskCreate()'.
*
* Returns    : OS_ERR_NONE              成功
*              OS_ERR_TASK_NO_MORE_TCB  "没有空闲TCB"错误
*                                       没有更多的任务控制块被分配，将无法创建新的任务。
*
* Note       : 这个函数是uC/OS-II系统内部调用的函数，你的应用程序不能调用它。
*********************************************************************************************************
*/
                   ...
                   INT16U   opt)
{
    ...
    ptcb = OSTCBFreeList;                                  /* 获取空TCB指针                            */
    if (ptcb != (OS_TCB *)0) {                             /* 如果缓冲区有空余TCB，这个TCB将被初始化   */
        OSTCBFreeList            = ptcb->OSTCBNext;        /* 更新空TCB列表指针（指向下一空TCB）       */
        OS_EXIT_CRITICAL();
        ptcb->OSTCBStkPtr        = ptos;                   /* 当前PCB栈顶的指针(输入)                  */
        ptcb->OSTCBPrio          = prio;                   /* 往TCB装入任务优先级(输入)                */
        ptcb->OSTCBStat          = OS_STAT_RDY;            /* 任务准备好运行（就绪态）                 */
        ptcb->OSTCBStatPend      = OS_STAT_PEND_OK;        /* 清除PEND状态                             */
        ptcb->OSTCBDly           = 0u;                     /* 任务不延时                               */

#if OS_TASK_CREATE_EXT_EN > 0u                             /* 若创建任务扩展功能                       */
        ptcb->OSTCBExtPtr        = pext;                   /* 存储TCB扩展指针                          */
        ptcb->OSTCBStkSize       = stk_size;               /* 存储堆栈大小                             */
        ptcb->OSTCBStkBottom     = pbos;                   /* 存储栈底指针                             */
        ptcb->OSTCBOpt           = opt;                    /* 存储任务选项                             */
        ptcb->OSTCBId            = id;                     /* 存储任务 ID                              */
#else
        pext                     = pext;                   /* 若未使用扩展功能，这里是防止编译警告     */
        stk_size                 = stk_size;
        pbos                     = pbos;
        opt                      = opt;
        id                       = id;
...
#if (OS_EVENT_EN)
        ptcb->OSTCBEventPtr      = (OS_EVENT  *)0;         /* 将事件控制块指向空                       */
#if (OS_EVENT_MULTI_EN > 0u)
        ptcb->OSTCBEventMultiPtr = (OS_EVENT **)0;         /* 将任务多事件控制块指向空                 */
#endif
#endif

#if (OS_FLAG_EN > 0u) && (OS_MAX_FLAGS > 0u) && (OS_TASK_DEL_EN > 0u)
        ptcb->OSTCBFlagNode      = (OS_FLAG_NODE *)0;      /* 将事件标志节点指向空                     */
#endif

#if (OS_MBOX_EN > 0u) || ((OS_Q_EN > 0u) && (OS_MAX_QS > 0u))
        ptcb->OSTCBMsg           = (void *)0;              /* 将消息指向空                             */
#endif

#if OS_TASK_PROFILE_EN > 0u
        ptcb->OSTCBCtxSwCtr      = 0uL;                    /* 初始化(性能分析)变量                     */
...
#if OS_TASK_REG_TBL_SIZE > 0u                              /* 初始化任务变量                           */
        ...
        OSTaskCreateHook(ptcb);                            /* 调用用户定义的Hook函数                   */
        ...
        OS_ENTER_CRITICAL();
        ptcb->OSTCBNext = OSTCBList;                       /* (TCB链表"Next")指向OSTCBList             */
        ptcb->OSTCBPrev = (OS_TCB *)0;
        if (OSTCBList != (OS_TCB *)0) {                    /* 如果不是首次创建任务···                  */
            OSTCBList->OSTCBPrev = ptcb;
        }
        OSTCBList               = ptcb;
        OSRdyGrp               |= ptcb->OSTCBBitY;         /* 使任务进入"就绪"状态                     */
        OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
        OSTaskCtr++;                                       /* 任务计数器 + 1                           */
        OS_EXIT_CRITICAL();
        return (OS_ERR_NONE);
