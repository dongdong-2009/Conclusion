...
;********************************************************************************************************
    ...
    EXTERN  OSRunning                                           ; 外部引用（全局变量、函数，类似于C语言中的extern）
    EXTERN  OSPrioCur
    ...
    EXPORT  OS_CPU_SR_Save                                      ; 函数声明（本文件）
...
NVIC_PENDSV_PRI EQU           0xFF                              ; 设置PendSV的可编程优先级值，即最低优先级。
NVIC_PENDSVSET  EQU     0x10000000                              ; 中断控制及状态寄存器ICSR的位28写1以悬起 PendSV中断。读取它则返回 PendSV 的状态。

;********************************************************************************************************
;                                      CODE GENERATION DIRECTIVES（代码生成指令）
;********************************************************************************************************
...
;********************************************************************************************************
;                       CRITICAL SECTION METHOD 3 FUNCTIONS（与中断方式3有关的两汇编函数）
;
...
;
; Note(s) : 1) 这个函数将触发一个PendSV异常(上下文切换)
;              启动第一个任务。
;
...
OSStartHighRdy
    LDR     R0, =NVIC_SYSPRI14                                  ; 装载系统异常优先级寄存器PRI_14, 即设置PendSV中断优先级的寄存器。
    LDR     R1, =NVIC_PENDSV_PRI                                ; 装载 PendSV的可编程优先级(255)。
    STRB    R1, [R0]                                            ; 无符号字节寄存器存储。R1是要存储的寄存器, 存储到内存地址所基于的寄存器。
...
;
; Notes:    1) OSIntCtxSw()将被OSIntExit()调用。
;              当确定需要上下文切换，需要禁止中断。
;              这个函数只是触发PendSV异常。
;********************************************************************************************************
