...
void DealModBus(void)
{
	...
	REG0X03[4]=(RTDtemperature[0]*10);        //chiller water outlet temperature
	...
	REG0X03[9]=(dataTemp[10]*10);      //valve feedback value
	REG0X03[10]=0;
	REG0X03[11]=0;
	...
	switch(Uart1_receive_buffer[1])     //功能码
	{
		...
		case 0x10:
							...
							{
								REG0X06[StartRegAdd+i]=Uart1_receive_buffer[7+2*i]*256+Uart1_receive_buffer[8+2*i];
							}
							Uart1_Send_buffer[sendLen++]=Uart1_receive_buffer[2];
							Uart1_Send_buffer[sendLen++]=Uart1_receive_buffer[3];
							Uart1_Send_buffer[sendLen++]=Uart1_receive_buffer[4];
...
//	PDVComm=REG0X06[0];
//	RoomTempSv=REG0X06[1];
//	RoomHumSv=REG0X06[2];
//	InletTemp=REG0X06[3];
//	ScheduleComm=REG0X06[4];
	for(i=0;i<8;i++)            //DA寄存器地址对于0x0001~0x0008
	{
		DAREG[i]=REG0X06[i];
	}
