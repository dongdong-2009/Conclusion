...
void GenApStateJson(WifiServer *server, WifiCfg *cfg)
{
	char buf[128];
	uint32_t len;
	
	strcpy(server->tmpBuf,"{");
	sprintf(buf,"\"ApState\":\"%d\",\"FailCnt\":\"%d\",\"bootConnToAp\":\"%d\",\"state\":\"%d\",\"connToAp\":\"%d\"",server->apState,server->failCnt,server->bootConnToAp,server->state,server->connToAp);
	strcat(server->tmpBuf,buf);
	strcat(server->tmpBuf,"}");
...
void WifiServer_IpdPro(WifiServer *server, WifiCfg *wifiCfg, WifiRecvStr *wifiRecv)
{
	...
	if (!strncmp(pdata, "GET ", 4)) 
	{
		if(flag == 0)
		{
			...
			{
				server->recvGetIp = 1;
				server->recvRequest = 1;
			}
			else if (strcmp(str1,"/ApState") == 0)
			{
				server->recvRequest = 2;
			}
	...
	{	
		...
		case WIFISERVER_RECV:
			...
			if(Wifi_GetIpdRecvState() == 0)
			{
				...
				{
					//10s查询一次ap状态
					if(IsTimeExpired(wifiServer.checkApStateTimer + 1000))//1000*10ms=10s
					{
				...
				else if (wifiServer.connToAp == 2)
				{
					//30s尝试重连
					if(IsTimeExpired(wifiServer.checkApStateTimer + 3000))//3000*10ms=30s
					{
						wifiServer.checkApStateTimer = Get_Gtime();
				...
				else if(wifiServer.recvRequest > 0)
				{
					wifiServer.send.socket = wifiRecvStr.socket;
					switch (wifiServer.recvRequest)
					{
						case 1:
							//GetIP
						...
						case 2:
							//ApState
							GenApStateJson(&wifiServer, &wifiCfg);
							break;
						default:
							break;							
					}
					wifiServer.recvRequest = 0;
