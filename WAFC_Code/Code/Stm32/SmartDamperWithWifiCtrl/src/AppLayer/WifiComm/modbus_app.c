...
RegTable MbTcpDataTbl[]=
{
	...
	{OFFSET(MbTcpAppData,Damper.Mode),						37,	2},
	...
	{OFFSET(MbTcpAppData,CO2.Bias),							82,	2},
	{OFFSET(MbTcpAppData,CO2.Limit),						83,	2},
	
	{OFFSET(MbTcpAppData,VentCtrl.Mode),				88,	2},
	{OFFSET(MbTcpAppData,VentCtrl.Kp_FlowSP),		89,	4},
	{OFFSET(MbTcpAppData,VentCtrl.MinFlow),			91,	4},
	
	{OFFSET(MbTcpAppData,PIDCtrl.U),						96,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.LB),						98,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.UB),						100,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.Kp),					102,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.Ki),					104,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.G0),						106,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.G1),						108,	4},
	{OFFSET(MbTcpAppData,PIDCtrl.FbSrc),				110,	2},
...
{
	// these values could be overwriten if exist flash storage.
	gModuleData.Sys.Version = 0;
	gModuleData.Sys.SubVersion = 1;
	
	gModuleData.Flow.MeasureSrc = FLOW_MEASURE_SRC_PITOT;
	gModuleData.Flow.SetPointSrc = FLOW_SP_SRC_MANUAL;
	
	gModuleData.Damper.Mode = DAMPER_MODE_TRACKING;
	gModuleData.Damper.SetPointSrc = DAMPER_SP_SRC_MANUAL;
	gModuleData.Damper.SetPoint = 45.0;
	
	gModuleData.Servo.Mode = SERVO_MODE_TRACKING;
	gModuleData.Servo.PWM_Angle_0 = 1500;
	gModuleData.Servo.PWM_Angle_90 = 1500;
		
	gModuleData.CO2.MeasureSrc = CO2_MEASURE_SRC_SENSOR;
	
	gModuleData.VentCtrl.Mode = VENTCTRL_MODE_MANUAL;
	
	gModuleData.PIDCtrl.FbSrc = FEEDBACK_SRC_FLOW;
}
