...
/**
  ...
  */
uint32_t Wifi_CheckIpstatus(uint32_t *linkstatus)
{
	uint32_t atCmdRet;
	EspATcmdStr atCmdInfo;
	char *pos = NULL;
...
/**
  ...
  * @retval 0:ok;1:fail
  */
uint32_t Wifi_ConntoAp(char *ssid,char *pwd,uint32_t port,uint32_t *local_ip)
{
	uint32_t atCmdRet;
	...
	switch(wifiPro.conntoap_state)
	{
		case WIFI_CONNTOAP_CJWAP:
			atCmdRet = Wifi_Set_Cwjap(ssid,pwd);
			if(atCmdRet == WIFI_RET_OK)
			{
		...
		case WIFI_CONNTOAP_TCPSERVER:
			//建立tcp服务器
			atCmdRet = Wifi_SetIpServer(1,port);
			if(atCmdRet == WIFI_RET_OK)
			{
			...
			{
				wifiPro.conntoap_state = WIFI_START_STATION;
				return atCmdRet;
			}
			break;
		case WIFI_CONNTOAP_GETIP:
			atCmdRet = Wifi_GetIPAddr(local_ip);
			if(atCmdRet == WIFI_RET_OK)
			{
				wifiPro.conntoap_state = WIFI_CONNTOAP_CJWAP;
				return WIFI_RET_OK;
