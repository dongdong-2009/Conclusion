...
void Servo_Position(float angle) // direct adjust 
{
	...
	PWM_open = (int)gModuleData.Servo.PWM_Angle_0;
	PWM_close = (int)gModuleData.Servo.PWM_Angle_90;
	if (angle<ANGLE_MIN)
		angle = ANGLE_MIN;
	else if (angle>ANGLE_MAX)
		angle = ANGLE_MAX;
	PWM_Now = PWM_open+(int)(angle/ANGLE_RANGE*(PWM_close-PWM_open));
	//If considering hysteresis, PWM_Now should be adjusted
	
	
	TIM_SetCompare1(TIM3,(u16)PWM_Now);
	gModuleData.Servo.PWM = (u16)PWM_Now;
}
...
void Servo_Task(void)
{
	...
	if (IsTimeExpiredHD(_servo_Step_Timer))
	{
		...
		switch (gModuleData.Servo.Mode) /*******************incomplete***************************************/
		{
