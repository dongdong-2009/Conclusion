...
unsigned int _Buf_p; // _DPBuf_p<DP_BUF_SIZE
...
void DiffPressure_Init(void)
{
	...
	_DP_Output_Timer = HDTimer_add_ms(_DP_Output_Timer,DP_OUTPUT_CLK_MS); 
}
...
float DiffPressure_Filter(float buf[])
{
	...
	r = 0.0;
	for (i=0;i<DP_BUF_SIZE;i++)
		r += buf[(_Buf_p-i+DP_BUF_SIZE)%DP_BUF_SIZE]*_weight[i];
	return r;
}
...
float DiffPressure_Variance(float buf[], float buf_weighted_mean)
{
	...
	for (i=0;i<DP_BUF_SIZE;i++)
	{
		e = buf[(_Buf_p-i+DP_BUF_SIZE)%DP_BUF_SIZE] - buf_weighted_mean;
		r += e*e*_weight[i];
	}
...
void DiffPressure_Task(void)
{
	...
	else if (_DP_status == DP_STATUS_EXEMCU)
	{
		...
		{
			if((++_Buf_p)>=DP_BUF_SIZE) _Buf_p=0;
			_pressure_Buf[_Buf_p] = D6F_PH_Pressure();
			_temperature_Buf[_Buf_p] = D6F_PH_Temperature();
