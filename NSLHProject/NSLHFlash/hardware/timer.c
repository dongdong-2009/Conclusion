...
void Timer1Config(void)
{
	...
	TIM_TimeBaseStructure.TIM_Period = 20;                            // 每T秒更新一次子程序 T5<30
...
//定时往FLASH中存储数据,定时1s,往Flash中存储数据
void Tim5Configure(void)
{
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM5,ENABLE);
	
	TIM_DeInit(TIM5);                    					                    // 复位TIMx定时器 
	TIM_TimeBaseStructure.TIM_Period = 2000;                            // 每T秒更新一次子程序 T5<30
	TIM_TimeBaseStructure.TIM_Prescaler =(36000-1);                        // 分频36000       
  TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;           // 时钟分频  
  TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;       // 计数方向向上计数
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;                  // 重复计数  
  ...
  TIM_ITConfig(TIM5, TIM_IT_Update, ENABLE);                         //使能TIMx中断源 
	TIM_Cmd(TIM5, ENABLE); 
}
