...
int main(void)
{
	...
	Tim5Configure();               	//¶¨Ê±1sÍùFlashÖÐ´æÈëÊý¾Ý
	...
	//LC512_Write(WriteBuf,0x00,FLASHLENGTH);
	LC512_Read(ReadBuf,0x00,11);
	if(ReadBuf[10]==0xAA)
	{
		FlashDataRead();
	}
		
//	LC512_WriteByte(0x05,0x07);
//	ReadTemp = LC512_ReadByte(0x05); 
...
{
	RoomTempSv=ReadBuf[0];
	TempSPSet_Unocc=ReadBuf[1];	
	ERVCmd=ReadBuf[2];
	IBSCommand=ReadBuf[3];
	ScheduleComm=ReadBuf[4];
	PurgeCmd=ReadBuf[5];
	ValveCmd=ReadBuf[6];
	PDVComm=ReadBuf[7];
	RunStatus=ReadBuf[8];
