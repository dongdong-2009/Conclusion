...
//----------------------------------------------------------------------------------------------------------------------------------------   
    ...
    Setup_ADS1220 (ADS1220_MUX_SHORTED, ADS1220_OP_MODE_NORMAL,
                   ADS1220_CONVERSION_SINGLE_SHOT, ADS1220_DATA_RATE_20SPS, ADS1220_GAIN_32, ADS1220_USE_PGA,
                   ADS1220_IDAC1_AIN3, ADS1220_IDAC2_AIN2, ADS1220_IDAC_CURRENT_250_UA, 3);//pt100 250ua, pt1000 50ua
    Setup_ADS1220 (ADS1220_MUX_SHORTED, ADS1220_OP_MODE_NORMAL,
                   ADS1220_CONVERSION_SINGLE_SHOT, ADS1220_DATA_RATE_20SPS, ADS1220_GAIN_32, ADS1220_USE_PGA,
                   ADS1220_IDAC1_AIN3, ADS1220_IDAC2_AIN2, ADS1220_IDAC_CURRENT_250_UA, 4);//pt100 250ua, pt1000 50ua
    calibrateCount = 0;
 ...
 //-----------------------------------------------------------------------------------------------------------------------------------   
    ...
    Setup_ADS1220 (ADS1220_MUX_AIN0_AIN1, ADS1220_OP_MODE_NORMAL,
                   ADS1220_CONVERSION_CONTINUOUS, ADS1220_DATA_RATE_20SPS, ADS1220_GAIN_32, ADS1220_USE_PGA,
                   ADS1220_IDAC1_AIN3, ADS1220_IDAC2_AIN2, ADS1220_IDAC_CURRENT_250_UA, 3); // pt100 250 pt1000 50
    Setup_ADS1220 (ADS1220_MUX_AIN0_AIN1, ADS1220_OP_MODE_NORMAL,
                   ADS1220_CONVERSION_CONTINUOUS, ADS1220_DATA_RATE_20SPS, ADS1220_GAIN_32, ADS1220_USE_PGA,
                   ADS1220_IDAC1_AIN3, ADS1220_IDAC2_AIN2, ADS1220_IDAC_CURRENT_250_UA, 4); // pt100 250 pt1000 50
 ...
 //--------------------------------------------------------------------------------------------------------------------------------------------------------------       
        ...
        if (avgCount_1 == SAMPLES)
        {
            ...
            if(!(P1IN & BIT2))
            {
                ...
                BSP_TURN_ON_LED1();
                //-------------------------------------------------------------------------------------------------
                ConfigAndSaved.ADS1220GainCorrection_1 = MeasuredGainCodeValue_Pt100_150R / rawData_1; // MeasuredGainCodeValue_Pt100_150R or MeasuredGainCodeValue_Pt1000_1K5
                SaveData();
                while(!(P1IN & BIT2));
                BSP_TURN_OFF_LED1();
            }
    ...
    if (ReadConversionData_2)
    {
        ...
        if (avgCount_2 == SAMPLES)
        {
            ...
            if(!(P1IN & BIT3))
            {
                GainCal_2 = 0;
                BSP_TURN_ON_LED2();
                ConfigAndSaved.ADS1220GainCorrection_2 = MeasuredGainCodeValue_Pt100_150R / rawData_2;
                SaveData();
                while(!(P1IN & BIT3));
                BSP_TURN_OFF_LED2();
            }
...
void rtd_process_3(void)
{
    if (ReadConversionData_3)
    {
        ...
        // from timing out by sending a NOP command every time the loop is processed.
//--------------------------------------------------------------------------------------------------------------------------------------------------------------        
        temperature_3 = (long)(interpolateTemperatureValue (code_3, PgaGainLevel_3, PT100) * 1000.0);    // Get the temperature and convert into millidegrees C
        ...
        if (avgCount_3 == SAMPLES)
        {
            ...
            if(!(P1IN & BIT6))
            {
                ...
                BSP_TURN_ON_LED3();
                //--------------------------------------------------------------------------------------------------------------------------------------------------------------   
                ConfigAndSaved.ADS1220GainCorrection_3 = MeasuredGainCodeValue_Pt100_150R / rawData_3;
                SaveData();
                while(!(P1IN & BIT6));
                BSP_TURN_OFF_LED3();
            }
...
void rtd_process_4(void)
{
    if (ReadConversionData_4)
    {
        ...
        // from timing out by sending a NOP command every time the loop is processed.
   //--------------------------------------------------------------------------------------------------------------------------------------------------------------        
        temperature_4 = (long)(interpolateTemperatureValue (code_4, PgaGainLevel_4, PT100) * 1000.0);    // Get the temperature and convert into millidegrees C
        ...
        if (avgCount_4 == SAMPLES)
        {
             ...
             if(!(P1IN & BIT7))
            {
                ...
                BSP_TURN_ON_LED4();
                //--------------------------------------------------------------------------------------------------------------------------------------------------------------   
                ConfigAndSaved.ADS1220GainCorrection_4 = MeasuredGainCodeValue_Pt100_150R / rawData_4;
                SaveData();
                while(!(P1IN & BIT7));
                BSP_TURN_OFF_LED4();
            }
