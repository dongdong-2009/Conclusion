...
*********************************************************************************************************
*/
#define  OS_TASK_OPT_NONE          0x0000u  /* 无操作选项                                              */
#define  OS_TASK_OPT_STK_CHK       0x0001u  /* 使能参数检查                                            */
#define  OS_TASK_OPT_STK_CLR       0x0002u  /* 使能清除堆栈                                            */
#define  OS_TASK_OPT_SAVE_FP       0x0004u  /* 使能保存浮点指针                                        */
#define  OS_TASK_OPT_NO_TLS        0x0008u  /* 使能不需要TLS                                           */
...
/*
*********************************************************************************************************
*                                         TASK CONTROL BLOCK（任务控制块TCB结构体）
*********************************************************************************************************
*/
...
typedef struct os_tcb {
    ...
    void            *OSTCBExtPtr;           /* 指向用户定义的TCB（扩展指针）                           */
    OS_STK          *OSTCBStkBottom;        /* 指向栈底的指针                                          */
    INT32U           OSTCBStkSize;          /* 任务堆栈的容量(在栈里的数量)                            */
    INT16U           OSTCBOpt;              /* 任务选项。用于OSTaskCreateExt()                         */
    ...
    void            *OSTCBMsg;              /* 从OSMboxPost() 或者 OSQPost()中得到的消息               */
#endif
...
#endif
    OS_FLAGS         OSTCBFlagsRdy;         /* 准备运行的事件标志任务                                  */
#endif

    INT32U           OSTCBDly;              /* 任务延时滴答 或 事件超时等待                            */
    INT8U            OSTCBStat;             /* 任务状态                                                */
    INT8U            OSTCBStatPend;         /* 任务悬挂状态                                            */
    INT8U            OSTCBPrio;             /* 任务优先级 (0 == highest)                               */
                                            /* 以下4句注释为优先级<64时; 大于64时，用高低4位来讨论了   */
...
#if OS_TASK_DEL_EN > 0u
    INT8U            OSTCBDelReq;           /* 定义用于表示是否删除该任务                              */
#endif

#if OS_TASK_PROFILE_EN > 0u                 /* 以下是OS_TCB性能分析相关参数                            */
    INT32U           OSTCBCtxSwCtr;         /* 用于切换任务的时间计数                                  */
    INT32U           OSTCBCyclesTot;        /* 用于统计滴答时钟                                        */
    INT32U           OSTCBCyclesStart;      /* 快速启动任务计数                                        */
    OS_STK          *OSTCBStkBase;          /* 指向开始任务堆栈的指针                                  */
    INT32U           OSTCBStkUsed;          /* 堆栈中使用的字节数量                                    */
#endif
...
OS_EXT  INT8U             OSPrioHighRdy;            /* 最高任务优先级                                  */
...
OS_EXT  OS_TCB           *OSTCBCur;                        /* 指向当前运行的TCB指针                    */
OS_EXT  OS_TCB           *OSTCBFreeList;                   /* 空闲TCB列表指针（指向OSTCBTbl[]）        */
OS_EXT  OS_TCB           *OSTCBHighRdy;                    /* 指向最高优先级的TCB指针                  */
OS_EXT  OS_TCB           *OSTCBList;                       /* 指向双向链表的TCB指针                    */
OS_EXT  OS_TCB           *OSTCBPrioTbl[OS_LOWEST_PRIO + 1u];         /* TCB优先级表（指向创建的TCB）   */
