...
*                       存的低地址向高地址增长。
*
*              prio     任务的优先级。每个任务必须只有唯一的优先级作为标识。
*                       数字越小，优先级越高。
*
                     ...
                     INT8U    prio)
{
...
#if OS_ARG_CHK_EN > 0u
    if (prio > OS_LOWEST_PRIO) {             /* 确保优先级在指定范围内                                 */
        return (OS_ERR_PRIO_INVALID);
    }
#endif
    OS_ENTER_CRITICAL();
    if (OSIntNesting > 0u) {                 /*确保我们不会从ISR创建任务                               */
        OS_EXIT_CRITICAL();
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* 确保这个任务优先级未被使用，即就绪态为0                */
        ...
        if (err == OS_ERR_NONE) {
            ...
            OS_ENTER_CRITICAL();
            OSTCBPrioTbl[prio] = (OS_TCB *)0;/* OS_TCBInit()初始化错误，把这个优先级让给其它人         */
            OS_EXIT_CRITICAL();
        }
        return (err);
    }
...
*
*              pext      指向用户提供的内存(指针)。这里是用作 TCB 的扩展。
*                        For example, this user memory can hold the contents of floating-point registers
*                        during a context switch, the time each task takes to execute, the number of times
*                        the task has been switched-in, etc.
*
*              opt       包含任务额外信息(或用户操作选项)。
...
*                        OS_TASK_OPT_STK_CLR      清除堆栈(创建任务时清除堆栈)
*                        OS_TASK_OPT_SAVE_FP      保存浮点指针
*                                                 (在进行上下文切换时)
*
* Returns    : OS_ERR_NONE                      创建任务成功(没有错误)
*              OS_ERR_PRIO_EXIST                "优先级已存在"错误
*                                               (每个任务必须要有独立的优先级).
*              OS_ERR_PRIO_INVALID              "优先级无效"错误(优先级数值超过了允许的最大值)
*                                               (i.e. > OS_LOWEST_PRIO)
*              OS_ERR_TASK_CREATE_ISR           "从ISR中创建任务"错误
*              OS_ERR_ILLEGAL_CREATE_RUN_TIME   不符合"safety critical"错误
*
*********************************************************************************************************
*/
/*$PAGE*/
                        ...
                        INT16U   opt)
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* 确保这个任务优先级未被使用，即就绪态为0                */
        ...
        OS_TaskStkClr(pbos, stk_size, opt);                    /* 清除使用到的堆栈                     */
#endif
...
*
* Arguments  : pbos     指向任务堆栈栈顶的指针.  If the configuration constant
*                       OS_STK_GROWTH is set to 1, the stack is assumed to grow downward (i.e. from high
*                       memory to low memory).  'pbos' will thus point to the lowest (valid) memory
*                       location of the stack.  If OS_STK_GROWTH is set to 0, 'pbos' will point to the
*                       highest memory location of the stack and the stack will grow with increasing
...
void  OS_TaskStkClr (OS_STK  *pbos,
                     ...
                     INT16U   opt)
{
    if ((opt & OS_TASK_OPT_STK_CHK) != 0x0000u) {      /* 如果"参数检查"被使能                        */
        if ((opt & OS_TASK_OPT_STK_CLR) != 0x0000u) {  /* 如果使能了"清除堆栈选项"                    */
#if OS_STK_GROWTH == 1u
            while (size > 0u) {                        /* 堆栈在内存中"从高到低"增长                  */
                size--;
                *pbos++ = (OS_STK)0;                   /* 从栈底向上清除                              */
            }
#else
            while (size > 0u) {                        /* 堆栈在内存中"从低到高"增长                  */
                size--;
                *pbos-- = (OS_STK)0;                   /* 从栈底向下清除                              */
            }
#endif
        }
    }
