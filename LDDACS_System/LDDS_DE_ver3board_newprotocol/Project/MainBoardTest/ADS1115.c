...
float ReadAdOfChannel(unsigned char  Channel,unsigned char CurrentFlag)
{
	...
	switch(CurrentFlag)
	{ 
		case 0: resaultVoltageOrCurrent=0.256*51*resault/32767.0;break;    //FS=+-0.256
		case 1: resaultVoltageOrCurrent=4.096*1000*resault/32767.0/200.0;break;//FS=+-4.096
...
void ADC_allchannels(float *adc_value,char* flag)
{
	...
	for(i = 0; i < 12; i++)
	{
	  adc_value[i]= ReadAdOfChannel(i,flag[i]);
	//  ReadAdOfChannel(unsigned char  Channel,unsigned char CurrentFlag)
	  delay_us(2000);
	}
}
//-----------------------------------------------//
