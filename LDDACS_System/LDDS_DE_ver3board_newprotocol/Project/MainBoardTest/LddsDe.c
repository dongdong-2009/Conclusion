...
{  
    ...
    if(ADC_value[DELEVEL]>=DeSLevelHigh)//检查液位
    {
        return 0x01;
    }
    if(ADC_value[DELEVEL]<=DeSLevelLow)
    {
        return 0x02;
    }
    if(ADC_value[BUFLEVEL]>=StrSLevelHigh)
    {
        return 0x01;
    }
	if(ADC_value[BUFLEVEL]<=StrSLevelLow)
    {
        return 0x02;
    }
...
short int deh_transfer(void)//换液 等算法
{
   int dehlevelA=0; 
	...
	if(tempRegisters[DELEVEL]>=DeSLevelHigh)
    {
        return 0x01;
    }
   if(tempRegisters[DELEVEL]<=DeSLevelLow)
    {
        return 0x02;
    }
    if(ADC_value[BUFLEVEL]>=StrSLevelHigh)
	...
	if(tempRegisters[DELEVEL]>=DeSLevelTrasf)	
	{
   	  dehlevelA= highlevel_alarm();
	  if(dehlevelA!=0) 
	  return dehlevelA;	                                        
	}
   if(tempRegisters[DELEVEL]<DSLEVELSTOP)						                
	{
...

{
    ...
    if(tempRegisters[DELEVEL]>=DeSLevelHigh)//检查液位
    {
        return 0x01;
    }
    if(tempRegisters[DELEVEL]<=DeSLevelLow)
	...
	while(tempRegisters[DELEVEL]<=16.0)//检查液位是否小于22厘米
	{
...
char dehStop(PARAMS *arin,PARAMS *arout,char *dout)
{
	 ...
	 setAOonechanelPhyVal(CPUMP,0,arout[CPUMP].a);
     delay_s(5);
	 setAOonechanelPhyVal(SFAN,0,arout[SFAN].a);
	 delay_s(5);
...
float opencnts(float error)
 ...
 {	
	...
	htank =  ADC_value[BUFLEVEL]/100	;
	 t = (error/0.4)*65/sqrt(htank);
     return t ;  
 }

char alarmanaysis(void)
{
  char flag=0;
  u8 type=0;

  type = (int)tempRegisters[CASEEMERGENCY];

  if((alarmFlag==0)&(tempRegisters[CASEEMERGENCY]==0))
  return 0;
  
  flag =alarmFlag;

  if(tempRegisters[CASEEMERGENCY]!=0.0)
  {
     switch(type)
	 {
	  case 0x01:
