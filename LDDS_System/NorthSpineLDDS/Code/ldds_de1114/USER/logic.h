...
// preset value
#define DSLEVELSTART		 30 //除湿换液开始	33
...
#define SSLEVELMINALARM		 1 //STRONG SOLUTION LOW LIMITED WARNING
...
#define   TWIN		           14  //CHILL WATER Inlet温度
...
u8	 highlevel_alarm(void)		   ;
u8 lowlevel_sv_alarm(void)	  ;
...
float SolInletPID(float aimed_solultiontemp,float arout[]);
...
extern u8 timer4open;
extern float SetpointValueL[2];
int valveopen(float t);

void deInit(float arr[8]);
void deClever(float aout[8],u8 dout[8]);
void deDefault(void);
void getlevel(float delevel,float sslevel,float chiltemp,float cwfb,float outat);
