...
{ 
	...
	{
		if(lcddev.id==0x9341)  //9341在8位模式下写寄存器和寄存器数据时，只写一个8位，其他IC要写2个
		{
...
void LCD_WR_ERGDATA9341(u8 data) //9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
{
	if(lcddev.bus16==1) //16位接口
	{
		LCD->LCD_RAM=data;
...
void LCD_WR_REG_DATA(u8 LCD_Reg, u16 LCD_RegValue)
{
	...
	if(lcddev.id==0x9341)
	{
		LCD_WR_ERGDATA9341(LCD_RegValue);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
	}
	else
	{
...
{	 
	...
	{		    
		...
		LCD_WR_ERGDATA9341(Xpos>>8); //9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(Xpos&0XFF);	 //9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_REG(lcddev.setycmd); 
		LCD_WR_ERGDATA9341(Ypos>>8); //9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(Ypos&0XFF);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
...
{ 	
	...
	}else if(lcddev.id==0X8989)
	{
		...
		lcddev.setxcmd=0X4E;
		lcddev.setycmd=0X4F;  
	}else if(lcddev.id==0x0090||lcddev.id==0x0070||lcddev.id==0x0050)
	{
		lcddev.width=800;
		lcddev.height=480;
		lcddev.setxcmd=0x03;
  ...
  // Display Setting
	...
	{	 
		LCD_WR_REG(0xcf); 
		LCD_WR_ERGDATA9341(0x00);  //9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(0xc1);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(0x30);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别

		LCD_WR_REG(0xed); 
		LCD_WR_ERGDATA9341(0x64);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(0x03);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(0x12);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别
		LCD_WR_ERGDATA9341(0x81);//9341写寄存器数据专用函数,只在8位模式下跟其他IC有区别，16位模式下没有区别

		LCD_WR_REG(0xcb); 
		LCD_WR_ERGDATA9341(0x39);
		LCD_WR_ERGDATA9341(0x2c);
		LCD_WR_ERGDATA9341(0x00);
		LCD_WR_ERGDATA9341(0x34);
		LCD_WR_ERGDATA9341(0x02);
		...
		LCD_WR_REG(0xe8); 
		LCD_WR_ERGDATA9341(0x85);
		LCD_WR_ERGDATA9341(0x10);
		LCD_WR_ERGDATA9341(0x79);

		LCD_WR_REG(0xC0); //Power control
		LCD_WR_ERGDATA9341(0x23); //VRH[5:0]

		LCD_WR_REG(0xC1); //Power control
		LCD_WR_ERGDATA9341(0x11); //SAP[2:0];BT[3:0]

		LCD_WR_REG(0xC2);
		LCD_WR_ERGDATA9341(0x11);

		LCD_WR_REG(0xC5); //VCM control
		LCD_WR_ERGDATA9341(0x3d);
		LCD_WR_ERGDATA9341(0x30);

		LCD_WR_REG(0xc7); 
		LCD_WR_ERGDATA9341(0xaa);

		LCD_WR_REG(0x3A); 
		LCD_WR_ERGDATA9341(0x55);

		LCD_WR_REG(0x36); // Memory Access Control
		LCD_WR_ERGDATA9341(0x08);
		...
		LCD_WR_ERGDATA9341(0x0a);
		LCD_WR_ERGDATA9341(0xa2);

		LCD_WR_REG(0xF2); // 3Gamma Function Disable
		LCD_WR_ERGDATA9341(0x00);

		LCD_WR_REG(0xF7);
		LCD_WR_ERGDATA9341(0x20);

		LCD_WR_REG(0xF1);
		LCD_WR_ERGDATA9341(0x01);
		LCD_WR_ERGDATA9341(0x30);
		...
		LCD_WR_ERGDATA9341(0x0f);
		LCD_WR_ERGDATA9341(0x3f);
		LCD_WR_ERGDATA9341(0x2f);
		LCD_WR_ERGDATA9341(0x0c);
		LCD_WR_ERGDATA9341(0x10);
		LCD_WR_ERGDATA9341(0x0a);
		LCD_WR_ERGDATA9341(0x53);
		LCD_WR_ERGDATA9341(0xd5);
		LCD_WR_ERGDATA9341(0x40);
		LCD_WR_ERGDATA9341(0x0a);
		LCD_WR_ERGDATA9341(0x13);
		LCD_WR_ERGDATA9341(0x03);
		LCD_WR_ERGDATA9341(0x08);
		LCD_WR_ERGDATA9341(0x03);
		LCD_WR_ERGDATA9341(0x00);
		...
		LCD_WR_ERGDATA9341(0x03);
		LCD_WR_ERGDATA9341(0x0f);
		LCD_WR_ERGDATA9341(0x05);
		LCD_WR_ERGDATA9341(0x2c);
		LCD_WR_ERGDATA9341(0xa2);
		LCD_WR_ERGDATA9341(0x3f);
		LCD_WR_ERGDATA9341(0x05);
		LCD_WR_ERGDATA9341(0x0e);
		LCD_WR_ERGDATA9341(0x0c);
		LCD_WR_ERGDATA9341(0x37);
		LCD_WR_ERGDATA9341(0x3c);
		LCD_WR_ERGDATA9341(0x0F);
	...
	{
		 LCD_WR_REG(0x0011);LCD_WR_DATA(0x001A);
	     LCD_WR_REG(0x0012);LCD_WR_DATA(0x3121);
	     LCD_WR_REG(0x0013);LCD_WR_DATA(0x006C);
	     LCD_WR_REG(0x0014);LCD_WR_DATA(0x4245);
	     LCD_WR_REG(0x0010);LCD_WR_DATA(0x0800);
	     delay_ms(10);
	     LCD_WR_REG(0x0011);LCD_WR_DATA(0x011A);
	     delay_ms(10);
	     LCD_WR_REG(0x0011);LCD_WR_DATA(0x031A);
	     delay_ms(10);
	     LCD_WR_REG(0x0011);LCD_WR_DATA(0x071A);
	     delay_ms(10);
	     LCD_WR_REG(0x0011);LCD_WR_DATA(0x0F1A);
	     delay_ms(20);
	     LCD_WR_REG(0x0011);LCD_WR_DATA(0x0F3A);
	     delay_ms(30);
	     /*Initialization set sequence*/
	     LCD_WR_REG(0x0001);LCD_WR_DATA(0x011C);
	     LCD_WR_REG(0x0002);LCD_WR_DATA(0x0100);
	     LCD_WR_REG(0x0003);LCD_WR_DATA(0x1030);
	     LCD_WR_REG(0x0007);LCD_WR_DATA(0x0000);
	     LCD_WR_REG(0x0008);LCD_WR_DATA(0x0808);
	     LCD_WR_REG(0x000B);LCD_WR_DATA(0x1100);
	     LCD_WR_REG(0x000C);LCD_WR_DATA(0x0000);
	     LCD_WR_REG(0x000F);LCD_WR_DATA(0x1401);
	     LCD_WR_REG(0x0015);LCD_WR_DATA(0x0000);
	     LCD_WR_REG(0x0020);LCD_WR_DATA(0x0000);//RAM Address Register AD7-AD0
	     LCD_WR_REG(0x0021);LCD_WR_DATA(0x0000);//RAM Address Register AD15-AD8
	
	     LCD_WR_REG(0x0038);LCD_WR_DATA(0x00DB);
	     LCD_WR_REG(0x0039);LCD_WR_DATA(0x0000);
	     LCD_WR_REG(0x0050);LCD_WR_DATA(0x0001);//Gamma
	     LCD_WR_REG(0x0051);LCD_WR_DATA(0x020B);
	     LCD_WR_REG(0x0052);LCD_WR_DATA(0x0805);
	     LCD_WR_REG(0x0053);LCD_WR_DATA(0x0404);
	     LCD_WR_REG(0x0054);LCD_WR_DATA(0x0C0C);
	     LCD_WR_REG(0x0055);LCD_WR_DATA(0x000C);
	     LCD_WR_REG(0x0056);LCD_WR_DATA(0x0101);
	     LCD_WR_REG(0x0057);LCD_WR_DATA(0x0400);
	     LCD_WR_REG(0x0058);LCD_WR_DATA(0x1108);
	     LCD_WR_REG(0x0059);LCD_WR_DATA(0x050C);
	     
	     LCD_WR_REG(0x0036);LCD_WR_DATA(0x00AF);//H end
	     LCD_WR_REG(0x0037);LCD_WR_DATA(0x0000);//H start
	     ...
	     LCD_WR_REG(0x0007);LCD_WR_DATA(0x0016);
	     delay_ms(2);
	...
	{			      
		...
		LCD_WR_REG_DATA(0X07,0X0033);   
		LCD_WR_REG_DATA(0X07,0X0133);   
	}else if(lcddev.id==0x0043||lcddev.id==0x0050||lcddev.id==0x0070||lcddev.id==0x0090)	 //MD070SD OR MD050SD
	{
		//工业级总线型模块，
